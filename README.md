...
- 추가된 링크는 새 공지로 처리하고, 제거된 링크는 공지와 체크리스트를 삭제 상태로 기록해 DM에서 제외합니다. 다른 원본에 이미 등록된 중복 링크는 가져오거나 덮어쓰지 않습니다.
...
원본 동기화를 처음 적용할 때는 봇을 중지한 뒤 `.venv/bin/alembic upgrade head`를 실행하고 다시 시작합니다. 최신 revision은 `c83021fb573d`입니다. 이미 발송된 개인 DM에서 공지 내용 수정, 링크 추가·제거, 원문 삭제를 순서대로 확인합니다.
