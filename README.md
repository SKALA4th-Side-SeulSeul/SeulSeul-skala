...
DBeaver 접속은 [현재 상태와 SSH 터널 절차](docs/DB-ACCESS.md)를 따릅니다. 운영 Compose는 서버의 `127.0.0.1:15432`를 컨테이너의 PostgreSQL `5432`에 연결합니다. DBeaver는 SSH 터널을 통해 이 루프백 포트에 접속하며, 어떤 포트든 외부 인터페이스에 공개하지 않습니다. SSH 계정·인증 및 조회 전용 DB 권한은 실제 설정 확인이 필요합니다.
...
- `./view.sh logs`는 과거 로그만 출력하고, `./view.sh logs --follow`는 최근 로그를 먼저 보여준 뒤 실시간 추적을 시작합니다. `--tail N`으로 과거 범위를 늘릴 수 있으며, `Ctrl+C`는 실시간 로그 보기만 종료합니다. 기본 로그에는 가입자의 Slack 이름이 표시될 수 있고, 원본 로그에는 진단 식별자와 개인정보가 포함될 수 있으므로 공유 전 가려야 합니다.
...
2. 마지막 줄에서 Enter를 누른 뒤 빈 입력 줄에서 **Ctrl+D**를 눌러 미리보기를 확인합니다. Enter만 누르면 문단 구분용 빈 줄이 추가됩니다.
3. 워크스페이스·수신 인원·메시지를 확인하고 `y`를 입력하면 발송합니다. Enter 또는 확인 전 Ctrl+C는 취소합니다.

터미널에서 직접 실행하세요. Ctrl+D 이후 발송 확인을 입력할 수 있도록 컨테이너에 TTY를 연결하며 파일·파이프 입력은 지원하지 않습니다. 확인 화면에서 Ctrl+D를 다시 누르면 취소합니다.

봇 토큰이 속한 워크스페이스에서 현재 가입한 학생에게 `[SeulSeul 운영 안내]` 제목의
별도 일반 DM을 보냅니다. 기존 체크리스트를 수정하지 않으며, 발송 직전 해지 여부를 다시 확인합니다.
확인 이후 새로 가입한 학생은 이번 대상에 포함하지 않습니다.
학생별로 순차 발송하며 성공·실패·결과 불명·해지 제외·미발송 인원을 표시합니다.
Slack 전송 제한이 발생하면 남은 발송을 중단합니다.

예약·자동 발송·자동 재시도·영속 발송 이력은 제공하지 않습니다.
통신 오류나 발송 도중 중단된 요청은 실제 전달 여부가 불명확할 수 있습니다.
**다시 실행하면 이전에 성공한 학생에게도 재발송되므로 결과를 확인하고 실행하세요.**
기존 시작·해지의 봇 DM 정리 정책에 따라 운영 안내 DM도 이후 정리될 수 있습니다.
