...
usage() {
    echo '사용법: ./view.sh [status|logs [bot|oauth|ngrok|postgres|all]|db|schema|config|--help]'
...
case "$operation" in
    ...
    status|db|schema|config)
        [[ $# -le 1 ]] || { usage; exit 2; } ;;
    logs)
        case "${2:-bot}" in bot|oauth|ngrok|postgres|all) ;; *) usage; exit 2 ;; esac ;;
    *) usage; exit 2 ;;
esac
require_operations
case "$operation" in
    ...
    logs)
        if [[ "${2:-bot}" == all ]]; then
            compose logs --tail 100 -f bot oauth ngrok postgres
        else
