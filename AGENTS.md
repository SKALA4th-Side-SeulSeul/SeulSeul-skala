...
- 간편 수동 수정: 운영 서버에서 `./notice_edit.sh`로 저장된 공지를 번호 선택하고 바꿀 분석값만 입력합니다. 원문·링크·완료 기록 보존, AI 호출 없음, 마지막 `y` 확인 후 저장. 단일 링크 원문만 지원하고 원본 잠금 아래 선택 당시 기록을 비교해 경합 시 중단합니다. DB·manifest 변경 없이 기존 CLI도 유지합니다.
