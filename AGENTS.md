...
- 언어: Python 3.11
- 마감 검증: `금일`은 `오늘`과 동일하게 Asia/Seoul 기준 Slack 최초 게시일 당일로 해석합니다. 수정·재처리 시각을 기준으로 바꾸지 않습니다.
...
- 운영 DB 외부 조회: `docs/DB-ACCESS.md`. 루프백 바인딩과 SSH 터널만 사용합니다. 운영 PostgreSQL 호스트 포트는 15432, 컨테이너 내부 포트와 봇 주소는 5432로 유지하며, 실제 SSH 인증·DB 조회 권한은 별도로 확인합니다. 전용 키 계정은 선택 구성안이며 서버 적용 완료로 가정하지 않습니다.
