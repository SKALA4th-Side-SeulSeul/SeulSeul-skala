...
- 학생 화면: 시작·해지 명령만 사용, 최초 연결 시 개인 DM 한 번 발송 후 같은 메시지 갱신 (D-025), 테두리 카드 묶음·굵은 짧은 제목 링크·📝 폼/📄 문서·작은 도메인/마감/원문·✓ 완료/↶ 취소, payload가 가리키는 본인 발송 기록 검증, 하단 단일 목록 전환·새로고침·페이지 이동
...
- 버튼 진단: `checklist_*` JSON 로그의 `trace_id`로 수신·검증·결과를 연결합니다. 발송 기록 없음/소유권/채널/메시지 시각 불일치를 구분하며 Slack 식별자는 해시 참조로 기록합니다. 로그 수집 절차는 README의 체크리스트 버튼 오류 로그 확인을 따릅니다.
